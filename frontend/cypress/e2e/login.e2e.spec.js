...
describe('Login E2E Tests', () => {
  ...
  it('Should show validation message "Password cannot be empty." for empty password', () =>{
    LoginPage.fillUser('admin');
    LoginPage.clickLogin();
    LoginPage.getPasswordValidation().should('be.visible').and('have.text', 'Password cannot be empty.');
  });

  const invalidPasswordCases = [
      { password: '123', message: 'Password is too short.' },
      { password: 'averylongpasswordthatexceedslimitaverylongpasswordthatexceedslimitaverylongpasswordthatexceedslimitaverylongpasswordthatexceedslimit',
  ...
  invalidPasswordCases.forEach(({password, message}) =>{
    it(`Should show validation message "${message}" for password:${password}`, () =>{
      ...
      LoginPage.clickLogin();
      LoginPage.getPasswordValidation().should('be.visible').and('have.text', message);
    });
  });
  ...
  it('Should show validation message "Username cannot be empty." for empty username', () =>{
    LoginPage.fillPassword('@Admin123')
    LoginPage.clickLogin();
    LoginPage.getUsernameValidation().should('be.visible').and('have.text', 'Username cannot be empty.');
  });

  const invalidUsernameCases = [
      { username: 'a', message: 'Username is too short.' },
      { username: 'averylongpasswordthatexceedslimitaverylongpasswordthatexceedslimitaverylongpasswordthatexceedslimitaverylongpasswordthatexceedslimit',
  ...
  invalidUsernameCases.forEach(({username, message}) =>{
    it(`Should show validation message "${message}" for username:${username}`, () =>{
      ...
      LoginPage.fillPassword('@Admin123');
      LoginPage.clickLogin();
      LoginPage.getUsernameValidation().should('be.visible').and('have.text', message);
    });
  });
